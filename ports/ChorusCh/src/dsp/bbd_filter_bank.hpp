...
struct InputFilterBank
{
    ...
    void setDelta(float delta) noexcept { Aplus = expFromAngles(poleCorrAngle, delta); }
    ...
    {
        return x.countSubnormals() + Gcalc.countSubnormals() + rootCorr.countSubnormals() + poleCorr.countSubnormals() + gCoef.countSubnormals();
    }

    float debugMaxAbs() const noexcept
    {
        return std::max({ x.maxAbs(), Gcalc.maxAbs(), rootCorr.maxAbs(), poleCorr.maxAbs(), gCoef.maxAbs() });
    }
...
struct OutputFilterBank
{
    ...
    float calcH0() const noexcept { return -1.0f * gCoef.sumReal(); }
    ...
    int debugSubnormalCount() const noexcept
    {
        return x.countSubnormals() + Gcalc.countSubnormals() + gCoef.countSubnormals() + poleCorr.countSubnormals() + Amult.countSubnormals();
    }

    float debugMaxAbs() const noexcept
    {
        return std::max({ x.maxAbs(), Gcalc.maxAbs(), gCoef.maxAbs(), poleCorr.maxAbs(), Amult.maxAbs() });
    }
