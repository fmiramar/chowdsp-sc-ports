...
{
public:
    explicit CapacitorAlphaT(T value, T fs = static_cast<T>(48000.0), T alpha = static_cast<T>(1.0))
        : C_value(value)
        , fs(fs)
        , alpha(alpha)
        , b_coef((static_cast<T>(1) - alpha) / static_cast<T>(2))
        , a_coef((static_cast<T>(1) + alpha) / static_cast<T>(2))
    {
        calcImpedance();
    }
    ...
    void calcImpedance() override
    {
        wdf.R = static_cast<T>(1) / (((static_cast<T>(1) + alpha) * C_value) * fs);
        wdf.G = static_cast<T>(1) / wdf.R;
    }
...
private:
    ...
    T b_coef = static_cast<T>(0);
    T a_coef = static_cast<T>(1);
};
...
{
public:
    DiodeT(Next& n, T Is, T Vt = static_cast<T>(25.85e-3), T nDiodes = static_cast<T>(1)) : next(n)
    {
