...
public:
    ...
    void setDeferPropagation(bool shouldDefer) noexcept { deferPropagation = shouldDefer; }
...
{
public:
    explicit ScopedDeferImpedancePropagation(NodeTypes&... newNodes) : nodes(newNodes...)
    {
        std::apply([](auto&... node) { (node.setDeferPropagation(true), ...); }, nodes);
    }

    ~ScopedDeferImpedancePropagation()
    {
        std::apply([](auto&... node) { (node.setDeferPropagation(false), ...); }, nodes);
    }

private:
    std::tuple<NodeTypes&...> nodes;
};
...
{
public:
    explicit InductorT(T value, T fs = static_cast<T>(48000.0)) : L_value(value), fs(fs)
    {
        calcImpedance();
    }
    ...
    void calcImpedance() override
    {
        wdf.R = static_cast<T>(2) * L_value * fs;
        wdf.G = static_cast<T>(1) / wdf.R;
    }
...
{
public:
    DiodePairT(Next& next, T Is, T Vt = static_cast<T>(25.85e-3), T nDiodes = static_cast<T>(1))
        : next(next)
    {
        next.connectToParent(this);
    ...
    T reflected() noexcept
    {
        const auto lambda = static_cast<T>(chowdsp::signum(wdf.a));
        if constexpr (Quality == Good) {
            wdf.b = wdf.a + static_cast<T>(2) * lambda
                * (R_Is - Vt * chowdsp::Omega::omega4(logR_Is_overVt + lambda * wdf.a * oneOverVt + R_Is_overVt));
        } else {
            const auto lambda_a_over_vt = lambda * wdf.a * oneOverVt;
...
private:
    Next& next;
    T Is = static_cast<T>(1.0e-9);
    T Vt = static_cast<T>(25.85e-3);
    T twoVt = static_cast<T>(2) * Vt;
    T oneOverVt = static_cast<T>(1) / Vt;
...
public:
    ...
    void setClosed(bool shouldClose) noexcept
    {
        closed = shouldClose;
    }
